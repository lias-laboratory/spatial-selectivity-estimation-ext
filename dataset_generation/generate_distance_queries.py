...
def process_table(row, output_dir, conn_params):
    ...
    size_proportions = {
        0.1: int(proportion_count * 0.8),  # 80% with distance_min and distance_max < 10% of universe diagonal/2
        0.2: int(proportion_count * 0.1),  # 10% with distance_min and distance_max < 20% of universe diagonal/2
        0.5: int(proportion_count * 0.08), # 8% with distance_min and distance_max < 50% of universe diagonal/2
        1.0: int(proportion_count * 0.02), # 2% with distance_min and distance_max < 100% of universe diagonal/2
    }
    ...
    with open(output_file, mode='w', newline='', encoding='utf-8') as out_file:
        ...
        for size, count in size_proportions.items():
            ...
            while written_count < count:
                # Fetch random spatial objects from the database in batches
                ...
                for spatial_object in spatial_objects:
                    ...
                    # Calculate the distances from the spatial object to the points
                    distance_1 = spatial_object.distance(point1)
                    distance_2 = spatial_object.distance(point2)

                    # Ensure distance_min is always less than distance_max
...
if __name__ == "__main__":
    ...
    summary_csv = "generation_summary_distance.csv"
